...
namespace un {
    void generate_includes_jobs::operator()(job_worker* worker) {
        ...
        std::filesystem::path loc = _translationUnit->get_location();
        ...
        for (const auto& item : _translationUnit->get_includes()) {
        ...
        ss << "#include <" << _translationUnit->get_self_include() << ">" << std::endl;
        ss << "#endif" << std::endl;
        ss << " // END GENERATED INCLUDES" << std::endl;
        _buf->operator=(ss.str());
    }
    ...
    ) : _translationUnit(_translationUnit),
        _plan(plan),
        _unitGraphIndex(unitGraphIndex),
        _buf(std::move(buf)) {
    }
}
